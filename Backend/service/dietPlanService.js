...
export const subscribeToDietPlan = async (req, res) => {
  try {
    ...
    // Unsubscribe from current diet plan if any
    if (user.subscribedDietPlan) {
      await DietPlan.findByIdAndUpdate(user.subscribedDietPlan, {
        $pull: { subscribers: user._id },
      });
    }

    // Subscribe to new diet plan
    user.subscribedDietPlan = dietPlan._id;
    await user.save();

    dietPlan.subscribers.push(user._id);
    await dietPlan.save();
